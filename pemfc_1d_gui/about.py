...
#: This interface. ``MODEL_REPOSITORY`` is the library it runs on; they are
#: separate repositories, and the dialog names both so a user who finds one
#: can find the other.
REPOSITORY = f"{GITHUB_PROFILE}/1D-PEMFC-Model-GUI"
MODEL_REPOSITORY = f"{GITHUB_PROFILE}/1D-PEMFC-Model"
...
<p style='color:#555; margin-top:0'>interface {gui_version} &mdash; model pemfc_1d {model_version}</p>
...
<p>This window only configures and displays the model. The solver is the
separate <code>pemfc_1d</code> package, installed as a dependency; every number
shown here comes from it, and each run is written to disk in the same layout
that package's own command-line runner uses. Nothing in the model imports this
interface, so a bug here cannot change a number it produces.</p>
...
<tr><td style='color:#555'>Interface&nbsp;&nbsp;</td>
    <td><a href='{REPOSITORY}'>{GITHUB_USER}/1D-PEMFC-Model-GUI</a></td></tr>
<tr><td style='color:#555'>Model&nbsp;&nbsp;</td>
    <td><a href='{MODEL_REPOSITORY}'>{GITHUB_USER}/1D-PEMFC-Model</a></td></tr>
