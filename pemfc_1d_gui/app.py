...
from pemfc_1d.metrics import (create_run_directory, git_revision,
                              write_run_log)
...
class MainWindow(QMainWindow):
    ...
    def _save_run(self, outcome: RunOutcome, figures: dict) -> None:
        ...
        # The model is installed as a dependency, so its own source is not a
        # checkout and cannot answer "which revision produced this?". This
        # interface is the program that ran, so it supplies its own revision;
        # the model's released version is recorded by the log either way.
